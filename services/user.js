...
var services = {
	"/user":var store = deep.ocm({
		admin:deep.store.Mongo.create({
			collectionName:"user",
	...
	}, { protocol:"user", groups:"roles" }),

	"/register/?(s:key/s:email)/$":new deep.Store({
		protocol:"register",
		mail:{
			options:{
	...
	"/change-password/?(s:email)/$":new deep.Store({
